  ...
  const KeluarMasukBarang = () => {
    ...
    const handleChange = (e) => {
      ...
      setForm((prev) => {
        ...
        // ✅ ID dibuat SAAT NAMA DIISI
        if (name === "nama" && value && !prev.id && !isEdit) {
          const prefix = "B"; // Barang
          const filtered = data.filter((item) =>
            String(item.id).startsWith(prefix),
          );
          const nextNumber = filtered.length + 1;

          updatedForm.id = `${prefix}_${String(nextNumber).padStart(3, "0")}`;
        }
    ...
    const handleSave = async () => {
      ...
      try {
        ...
        if (isEdit) {
          await axios.put(`${API_URL}/keluar-masuk`, payload);
        } else {
          await axios.post(`${API_URL}/keluar-masuk`, payload);
        }
            ...
            <div style={layout.modal}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  // Validasi hanya kalau KELUAR
                  if (form.keluarmasuk === "keluar" && value > saldoAktif) {
                    value = saldoAktif;
                    alert(`Nominal tidak boleh lebih dari saldo (${saldoAktif})`);
                  }
